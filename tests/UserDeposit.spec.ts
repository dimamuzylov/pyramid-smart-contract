...
describe('pyramide.fc contract tests', () => {
    ...
    beforeEach(async () => {
        ...
        pyramide = blockchain.openContract(
            Pyramide.createFromConfig(
                { admin_addr: adminWallet.address, daily_percent: 0, min_days: 7, max_days: 365 },
                code,
            ),
        );
    ...
    it('user deposit should have exit code if deposit already done', async () => {
        ...
        await pyramide.sendUserDeposit(senderWallet.getSender(), toNano('0.5'), DEFAULT_DAYS);
        const sendDeposit = await pyramide.sendUserDeposit(senderWallet.getSender(), toNano('0.5'), DEFAULT_DAYS * 2);
    ...
    it('user deposit should have exit code if less than 1 TON', async () => {
        const senderWallet = await blockchain.treasury('sender');
        const sendDeposit = await pyramide.sendUserDeposit(senderWallet.getSender(), toNano('0.5'), DEFAULT_DAYS);
    ...
    it('user deposit should have exit code if more than 50 TON', async () => {
        const senderWallet = await blockchain.treasury('sender');
        const sendDeposit = await pyramide.sendUserDeposit(senderWallet.getSender(), toNano('50.5'), DEFAULT_DAYS);
    ...
    it('user deposit should have exit code if days less than min deposit days', async () => {
        const senderWallet = await blockchain.treasury('sender');
        const sendDeposit = await pyramide.sendUserDeposit(senderWallet.getSender(), toNano('1'), 6);

        console.log('user deposit should have exit code if days less than min deposit days');
        printTransactionFees(sendDeposit.transactions);
    ...
    it('user deposit should have exit code if days more than max deposit days', async () => {
        const senderWallet = await blockchain.treasury('sender');
        const sendDeposit = await pyramide.sendUserDeposit(senderWallet.getSender(), toNano('1'), 400);

        console.log('user deposit should have exit code if days more than max deposit days');
        printTransactionFees(sendDeposit.transactions);
